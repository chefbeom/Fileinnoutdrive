package ngrinder.order.verifyOrder
...
class TestRunner {
    protected static String baseUrl = System.getProperty('baseUrl', 'http://192.100.220.17:8080/orders')
    ...
    static void beforeProcess() {
        initProcess("order-verify-order")
    }
    ...
    @Test
    void test() {
        def createResponse = postJson('/orders', [
                productCode: prop('productCode', 'PLUS')
        ])
        assertStatus(createResponse)

        def createResult = new JsonSlurper().parseText(createResponse.getBodyText()) as Map
        assert createResult.result?.orderId

        def response = postJson('/orders/verify', [
                paymentId: prop('paymentId', 'REPLACE_WITH_PAYMENT_ID'),
                orderId  : createResult.result.orderId
        ])
