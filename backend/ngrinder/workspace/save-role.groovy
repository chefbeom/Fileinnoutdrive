package ngrinder.workspace.saveRole
...
class TestRunner {
    ...
    static void beforeProcess() {
        initProcess("workspace-save-role")
    }

    @BeforeThread
    void beforeThread() {
        grinder.statistics.delayReports = true
        loginEmail = prop('workspaceAdminLoginEmail', prop('adminLoginEmail', loginEmail))
        loginPassword = prop('workspaceAdminLoginPassword', prop('adminLoginPassword', loginPassword))
        login()
    ...
    @Test
    void test() {
        def response = postJson('/workspace/saveRole/' + propLong('workspaceIdx', 1L), [
                (propLong('member1Id', 2L)): prop('role1', 'READ'),
                (propLong('member2Id', 3L)): prop('role2', 'WRITE')
        ])
