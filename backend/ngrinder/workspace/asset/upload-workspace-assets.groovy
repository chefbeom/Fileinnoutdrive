package ngrinder.workspace.asset.uploadWorkspaceAssets
...
class TestRunner {
    ...
    static void beforeProcess() {
        initProcess("workspace-asset-upload-workspace-assets")
    }
    ...
    void test() {
        def payload = multipartPayload([
                [name: 'files', filename: prop('assetFile1', 'asset-1.png'), contentType: 'image/png', bytes: samplePngBytes()],
                [name: 'files', filename: prop('assetFile2', 'asset-2.png'), contentType: 'image/png', bytes: samplePngBytes()]
        ])
        def response = request.POST(
                fullUrl('/workspace/' + propLong('workspaceId', 1L) + '/assets'),
                (byte[]) payload.bytes,
                multipartHeaders(payload.boundary)
        )
