package ngrinder.workspace.asset.uploadEditorJsImage
...
class TestRunner {
    ...
    static void beforeProcess() {
        initProcess("workspace-asset-upload-editorjs-image")
    }
    ...
    void test() {
        def payload = multipartPayload([
                [name: 'image', filename: prop('editorImageFile', 'editor.png'), contentType: 'image/png', bytes: samplePngBytes()]
        ])
        def response = request.POST(
                fullUrl('/workspace/' + propLong('workspaceId', 1L) + '/assets/editorjs'),
                (byte[]) payload.bytes,
