package ngrinder.file.manage.listPage
...
class TestRunner {
    ...
    static void beforeProcess() {
        initProcess("file-manage-list-page")
    }
    ...
    @Test
    void test() {
        def response = get('/file/list/page', [
                parentId     : propLong('parentId', 0L),
                page         : propInt('page', 1),
                size         : propInt('size', 20),
                sortOption   : prop('sortOption', 'updatedAt,desc'),
                searchQuery  : prop('searchQuery', ''),
                extensionFilter: prop('extensionFilter', ''),
                sizeFilter   : prop('sizeFilter', ''),
                customMinSize: prop('customMinSize', ''),
                customMaxSize: prop('customMaxSize', ''),
                statusFilter : prop('statusFilter', '')
        ])
