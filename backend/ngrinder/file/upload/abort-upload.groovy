package ngrinder.file.upload.abortUpload
...
class TestRunner {
    ...
    void test() {
        def initResponse = postJson('/file/upload', [[
                ...
                parentId      : propLong('parentId', 0L),
                relativePath  : prop('relativePath', '/'),
                lastModified  : propLong('lastModified', System.currentTimeMillis())
        ]])
        assertStatus(initResponse)

        def chunks = new JsonSlurper().parseText(initResponse.getBodyText()) as List
        assert chunks && !chunks.isEmpty()
        def first = chunks[0] as Map

        def response = postJson('/file/upload/abort', [
                finalObjectKey : first.finalObjectKey,
                chunkObjectKeys: [first.objectKey]
        ])
