package ngrinder.notification.subscribe
...
class TestRunner {
    ...
    static void beforeProcess() {
        initProcess("notification-subscribe")
    }
    ...
    @Test
    void test() {
        def response = postJson('/notification/subscribe', [
                userIdx : propLong('notificationUserIdx', 1L),
                endpoint: prop('pushEndpoint', 'https://example.com/push'),
                keys    : [
                        p256dh: prop('pushP256dh', 'test-p256dh'),
                        auth  : prop('pushAuth', 'test-auth')
                ]
        ])
