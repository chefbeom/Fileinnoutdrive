package ngrinder.feater.uploadProfileImage
...
class TestRunner {
    protected static String baseUrl = System.getProperty('baseUrl', 'http://192.100.220.17:8080/feater')
    ...
    static void beforeProcess() {
        initProcess("feater-upload-profile-image")
    }
    ...
    @Test
    void test() {
        def payload = multipartPayload([
                [name: 'image', filename: prop('profileFileName', 'profile.png'), contentType: 'image/png', bytes: samplePngBytes()]
        ])
        def response = request.POST(fullUrl('/feater/settings/me/profile-image'), (byte[]) payload.bytes, multipartHeaders(payload.boundary))
