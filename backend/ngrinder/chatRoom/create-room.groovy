package ngrinder.chatRoom.createRoom
...
class TestRunner {
    protected static String baseUrl = System.getProperty('baseUrl', 'http://192.100.220.17:8080/chatRoom')
    ...
    static void beforeProcess() {
        initProcess("chat-room-create-room")
    }
    ...
    @Test
    void test() {
        def response = postJson('/chatRoom/create', [
                title             : prop('roomTitle', 'ngrinder-room'),
                participantsEmail : [prop('participantEmail1', 'user1@example.com'), prop('participantEmail2', 'user2@example.com')]
        ])
        assertStatus(response, [201, 200])
    }
}
