package ngrinder.game.openhexagon.state
...
class TestRunner {
    protected static String baseUrl = System.getProperty('baseUrl', 'http://192.100.220.17:8080/game/openhexagon')
    ...
    static void beforeProcess() {
        initProcess("game-openhexagon-state")
    }

    @BeforeThread
    void beforeThread() {
        grinder.statistics.delayReports = true
        loginEmail = prop('gameLoginEmail', prop('adminLoginEmail', loginEmail))
        loginPassword = prop('gameLoginPassword', prop('adminLoginPassword', loginPassword))
        login()
    ...
    @Test
    void test() {
        def response = get('/game/openhexagon/state')
