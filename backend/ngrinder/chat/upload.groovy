...
class TestRunner {
    protected static String baseUrl = System.getProperty('baseUrl', 'http://192.100.220.17:8080/chat')
    ...
    void test() {
        def payload = multipartPayload([
                [name: 'file', filename: prop('chatFileName', 'chat.txt'), contentType: 'text/plain', bytes: textBytes(prop('chatFileContent', 'ngrinder chat attachment'))]
        ])
        def response = request.POST(
                fullUrl('/chat/' + propLong('roomIdx', 1L) + '/upload'),
                (byte[]) payload.bytes,
